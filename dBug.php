...
class dBug {
	...
	function makeTDHeader($type,$header) {
		...
		echo "<tr".$str_d.">
				<td valign=\"top\" class=\"dBug_".$type."Key\">".$header."</td>
				<td>";
	}
	...
	function varIsArray($var) {
		...
		else echo "<tr><td>".htmlentities($this->error("array")).$this->closeTDRow();
	...
	function varIsObject($var) {
		...
		else echo "<tr><td>".htmlentities($this->error("object")).$this->closeTDRow();
		array_pop($this->arrHistory);
		echo "</table>";
	}
